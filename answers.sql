...
SELECT m.name, b.name, headquarters FROM models AS m JOIN brands AS b USING (brand_id) WHERE m.name = 'Mustang' AND b.name = 'Ford';
...
SELECT m.year, m.name, m.model_id, a.name FROM models AS m LEFT JOIN awards AS a ON winner_id = model_id WHERE m.year >= 1964;
...
SELECT b.name, founded FROM brands AS b
...
SELECT m.name, m.year FROM models as m WHERE m.year IN (
